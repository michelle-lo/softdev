...
app = Flask(__name__)
app.secret_key = os.urandom(32) #randomly generates an alphanumeric string
#print("secret key: " + str(app.secret_key))
...
def disp_loginpage():
    ...
    if request.method == 'POST': # triggered when the user clicks the log out button
        session.pop('username') #removes session data
        return render_template('login.html')
    else:
        try:
            #renders response using response template if user is logged in already
            return render_template(
                'response.html',
                result = 'true',
                username = session['username'] #stores session data (username)
            )
        except:
            #otherwise, if the user is not already logged in, render login.
            return render_template( 'login.html' )
